...
async function main() {
  ...
  const publicTokenAmount = hre.ethers.utils.parseEther("1199700"); // USD191,952 worth of PixelVerse tokens at USD0.16 each for guaranteed round

  const crowdsalePaymentDecimal = 6;
  const crowdsaleRate = hre.ethers.utils.parseEther("0.015");
  ...
  if (network === "mainnet") {
    ...
    crowdsaleOpeningTime = 1635850800; // 2 Nov 2021 07:00 PM GMT+8
    crowdsaleClosingTime = 1635865200; // 2 Nov 2021 11:00 PM GMT+8
  ...
  if (network !== "mainnet") {
    const SaleToken = await hre.ethers.getContractFactory("EjsToken");
    const saleTokenContract = SaleToken.attach(saleTokenAddress);

    await saleTokenContract.mint(publicVestingContract.address, crowdsaleTokenCap);
  }
}
